...
//  Project5
//
//  Created by Розалия on 23.07.2020.
    ...
    func submit(_ answer: String) {
        ...
        if isPossible(word: lowerAnswer) {
            if isOriginal(word: lowerAnswer) {
                ...
                showErrorMessage(title: "Word not recognised or not long enough", error: "You can't just make them up, you know!")
                }
            }else {
                showErrorMessage(title: "Word used already", error: "Be more original!")
            }
        }else {
            guard let title = title else {
                return
            }
            showErrorMessage(title: "Word not possible", error: "You can't spell that word from \(title)")
